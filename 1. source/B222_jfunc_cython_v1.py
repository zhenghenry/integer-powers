...
def compute_B222_jmat(filename):
	...
	out_filename = outputfolder + 'B222_Jfunc_'+str(float(k1))+'_' + str(float(k2)) + '_' + str(float(k3)) + '_' +'.h5'

	# Output the table to csv 
	with h5py.File(out_filename,'w') as h5_file:
		h5_file.create_dataset("jmat", data=Jtriantable)
		h5_file.close()
